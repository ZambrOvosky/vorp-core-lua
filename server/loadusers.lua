...
AddEventHandler('playerDropped', function()
	...
	if _users[identifier] and _users[identifier].GetUsedCharacter() then
		...
		if Config.PrintPlayerInfoOnLeave then
			print('[' ..
				os.date('%d/%m/%Y %H:%M:%S') .. '] Player ^2' .. steamName .. ' ^7steam:^3 ' .. identifier .. '^7 saved')
		end
...
RegisterNetEvent('vorp:playerSpawn', function()
	...
	if numCharacters <= 0 then
		...
		-- 	local userMaxChar = CheckMaxChar(identifier)
		-- 	if userMaxChar and userMaxChar > 1 then
		-- 		return TriggerEvent('vorp_GoToSelectionMenu', _source)
		-- 	end
